...
namespace SAutoCarry.Champions.Helpers
{
    public static class Tumble
    {
        ...
        {
            if ((PortAIO.Champions.Vayne.Program.Only2W) && target.GetBuffCount("vaynesilvereddebuff") == 1) // == 1 cuz calling this after attack which is aa missile still flying
                return Vector3.Zero;

            if (PortAIO.Champions.Vayne.Program.Wall)
            {
            ...
            if (PortAIO.Champions.Vayne.Program.sacMode == 0)
            {
            ...
            else if (PortAIO.Champions.Vayne.Program.sacMode == 1)
            {
        ...
        {
            if (PortAIO.Champions.Vayne.Program.DontSafeCheck)
                return vec;
            ...
            if (checkTarget)
            {
                ...
                if (((PortAIO.Champions.Vayne.Program.DontQIntoEnemies || target.IsMelee) && HeroManager.Enemies.Any(p => p.ServerPosition.To2D().Distance(vec) <= p.AttackRange + ObjectManager.Player.BoundingRadius + (p.IsMelee ? 100 : 0))) || vec.UnderTurret(true))
                    return Vector3.Zero;
            }
